...
export default class extends BaseComponent<CalendarProps, CalendarState> {
  ...
  protected view = () => {
    ...
    return (
      <div className={this.getPrefixClass('container')} style={style}>
        ...
        <div className={this.gpc('tag-head')}>
          <div className={this.gpc('tag-year')}>
            <Select
              data={yearData}
          ...
          {mode === 'month' && (
            <div className={this.gpc('tag-month')}>
              <Select
                data={dim1}
          ...
          <div className={this.gpc('tag-btns')}>
            <ButtonGroup
              mode={mode}
  ...
  private year = () => {
    ...
    return (
      <div className={this.getPrefixClass('year')}>
        {dim2.map((line) => (
          <div key={line[0].key} className={this.gpc('tag-row')}>
            {line.map((item) => (
              <div
                key={item.key}
              ...
              >
                {monthFullCellRender ? monthFullCellRender(item.date) : (
                  <>
                    <div className={this.gpc('tag-title')}>{item.value}</div>
                    <div className={this.gpc('tag-content')}>
  ...
  private month = () => {
    ...
    return (
      <div className={this.getPrefixClass('month')}>
        <div className={this.gpc('tag-head-row')}>
          {weeks.map((item) => (
            <div
              key={item.key}
              className={this.gpc('tag-col')}
            >
              {item.value}
            </div>
          ))}
        ...
        {data.map((line) => (
          <div
            className={this.gpc('tag-content-row')}
            key={line[0].key}
          >
            {line.map((item) => (
              <div
  ...
  private yearDateItemClass = (item: CalendarYearData) => {
    ...
    return this.classNames(this.gpc('tag-col'), {
      [this.gpc('tag-col-cur')]: colCur,
      [this.gpc('tag-col-selected')]: colSelected,
    });
  };
  ...
  private monthDateItemClass = (item: CalendarMonthData) => {
    ...
    return this.classNames(
      ...
      this.gpc('tag-item'),
      {
        [this.gpc('tag-item-notcur')]: !item.cur,
        [this.gpc('tag-item-hover')]: itemHover,
        [this.gpc('tag-item-cur-date')]: itemCurDate,
        [this.gpc('tag-item-selected')]: itemSelected,
      },
    );
  };
