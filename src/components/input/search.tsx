...
class Search extends Component<BaseInputProps, InputState> {
  render(): React.ReactNode {
    ...
    const handleSearch = (e: Event) => {
      onSearch && onSearch(ref.current.state.value, e);
    };
    ...
    const onKeyPress = (e) => {
      ...
      e.stopPropagation();
      if (e.which === 13) {
        onSearch && onSearch(ref.current.state.value, ref.current);
        onPressEnter && onPressEnter(ref.current.state.value, ref.current);
      }
    };
    return <Input {...this.props} ref={ref} onKeyPress={onKeyPress} suffix={<Icon type="search" style={{ cursor: 'pointer' }} onClick={handleSearch} />} />;
  }
}
