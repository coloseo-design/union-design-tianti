...
export abstract class Picker<
    ...
    protected openPopup = (obj?: ((state: S, props: P) => Partial<S>) | Partial<S>) => this.setState((state, props) => ({
      ...state,
      ...typeof obj === 'function' ? obj(state, props) : obj,
      popupVisible: true,
...
export abstract class PickerRange extends Picker<'range'> {
    ...
    protected handleSelectDate = (date: Dayjs): [Dayjs?, Dayjs?] => {
      ...
      if (position === 'start') {
        result[0] = date;
        result[1] = selectedDate[1];
      }

      if (position === 'end') {
        result[0] = selectedDate[0];
        result[1] = date;
      }
      return result;
    };
    ...
    protected onDateNext = () => {
      ...
      if (startSelectedDate && endSelectedDate) {
        this.closePopup(({ selectedDate }) => ({ date: selectedDate }), true);
        return;
      }
