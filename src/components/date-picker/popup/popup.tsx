...
export abstract class Popup<T extends PickerType> extends BaseComponent<
  PopupProps<T>
> {
  ...
  protected getSelectDate = (): Dayjs | undefined => {
    ...
    return Array.isArray(selectedDate)
      ? selectedDate[position === 'start' ? 0 : 1]
      : selectedDate;
  };

  protected singleClassItem = <T extends PopupData>(item: T): string => {
    const isCur = item.date.isSame(
      this.curDate.format(this.formatTag),
      this.cmpTag,
    );
    const temp = this.getSelectDate() ?? this.getDate();
    const isSelected = !!temp && item.date.isSame(temp.format(this.formatTag), this.cmpTag);
    const disabled = this.props.disabledValue?.(item.date) ?? false;

    if (disabled) {
      return this.classNames(this.gpc('item'), this.gpc('item-disabled-date'));
    }
    ...
    return this.classNames(this.gpc('item'), {
      ...
      [this.gpc('item-disabled-date')]: disabled,
    });
  };

  protected rangeClassItem = <T extends PopupData>(item: T): string => {
    const {
      date, selectedDate, hoverDate, position,
    } = this
      .props as PopupProps<'range'>;
    const [s, e] = date ?? [];
    ...
    if (startSelectedDate || endSelectedDate) {
      if (
        (startSelectedDate
          && item.date.isSame(
            startSelectedDate.format(this.formatTag),
            this.cmpTag,
          ))
        || (endSelectedDate
          && item.date.isSame(endSelectedDate.format(this.formatTag), this.cmpTag))
      ) {
        classNamesObj[this.gpc('item-hover')] = false;
      ...
      if (item.cur && hoverDate) {
        ...
        if (
          ...
          if (
            ...
            if (item.curLinePos?.includes('end')) {
              classNamesObj[this.gpc('item-border-end')] = true;
            }
          } else if (
            item.date.isSame(hoverDate.format(this.formatTag), this.cmpTag)
          ) {
            classNamesObj[this.gpc('item-border-end')] = true;
            if (item.curLinePos?.includes('start')) {
              classNamesObj[this.gpc('item-border-start')] = true;
            }
          } else if (
            item.date.isAfter(
            ...
            && item.date.isBefore(hoverDate.format(this.formatTag), this.cmpTag)
          ) {
            if (item.curLinePos?.includes('start')) {
              classNamesObj[this.gpc('item-border-start')] = true;
            } else if (item.curLinePos?.includes('end')) {
              classNamesObj[this.gpc('item-border-end')] = true;
        ...
        if (
          ...
          && endSelectedDate
          && hoverDate.isBefore(
            endSelectedDate.format(this.formatTag),
            this.cmpTag,
          )
        ) {
          if (
            ...
            && item.date.isAfter(hoverDate.format(this.formatTag), this.cmpTag)
          ) {
            if (item.curLinePos?.includes('start')) {
              classNamesObj[this.gpc('item-border-start')] = true;
            } else if (item.curLinePos?.includes('end')) {
      ...
      if (item.cur && startSelectedDate && endSelectedDate) {
        ...
        if (position === 'start' && hoverDate) {
          if (
            item.date.isSameOrBefore(
              endSelectedDate.format(this.formatTag),
              this.cmpTag,
            )
            && hoverDate.isSameOrBefore(
              item.date.format(this.formatTag),
              this.cmpTag,
            )
          ) {
            ...
            if (
              ...
              classNamesObj[this.gpc('item-bg-hover')] = false;
            }
          }
        }
        ...
        if (position === 'end' && hoverDate) {
          if (
            item.date.isSameOrAfter(
              startSelectedDate.format(this.formatTag),
              this.cmpTag,
            )
            && hoverDate.isSameOrAfter(
              item.date.format(this.formatTag),
              this.cmpTag,
            )
          ) {
      ...
      if (
        classNamesObj[this.gpc('item-selected')]
        && classNamesObj[this.gpc('item-border-start')]
      ) {
        if (position === 'start') {
          classNamesObj[this.gpc('item-border-left')] = false;
        }
        if (position === 'end') {
          classNamesObj[this.gpc('item-border-left')] = true;
        }
      ...
      if (
        classNamesObj[this.gpc('item-selected')]
        && classNamesObj[this.gpc('item-border-end')]
      ) {
        if (position === 'start') {
          classNamesObj[this.gpc('item-border-right')] = true;
        }
    ...
    if (
      (startDate
        && startDate.isAfter(item.date.format(this.formatTag), this.cmpTag))
      || (endDate
        && endDate.isBefore(item.date.format(this.formatTag), this.cmpTag))
    ) {
      item.disabled = true;
