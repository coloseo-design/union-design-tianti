    ...
    protected viewHead = () => {
      ...
      return (
        <PopupHead
          ...
          content={(
            <div>
              <div data-class="title hover" onClick={onYear}>{`${viewDate.get('year')}年`}</div>
              <div data-class="title hover" style={{ marginLeft: 5 }} onClick={onMonth}>{`${viewDate.get('month') + 1}月`}</div>
            </div>
                )}
        />
      );
      ...
      return (
        <div className={this.getClass('popupdate-normal')}>
          <div
            ...
            data-class="head row"
          >
            {this.weeks.map((item) => <div data-class="head col" key={item.key}>{item.value}</div>)}
          </div>
          {data.map((line) => (
            <div
              data-class="content row"
              key={line[0].key}
            >
              {line.map((item) => (
                <div
                  data-class="content col"
                  key={item.key}
