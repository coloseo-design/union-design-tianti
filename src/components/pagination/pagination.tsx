...
import { Select, SelectBaseData, SelectProps } from '@union-design/calendar/select';
...
export default class Pagination extends BaseComponent<PaginationProps, PaginationState> {
  ...
  protected view = () => {
    ...
    return (
      <div className={this.getPrefixClass('wrap')}>
        ...
        {this.handleItem().map((item) => (item.type === 'num' ? (
          <div
            ...
            className={this.classNames(this.gpc('item'), {
              [this.gpc('item-hover')]: current !== item.key,
              [this.gpc('item-selected')]: current === item.key,
            })}
          >
            {item.value}
          </div>
        ...
        <div className={this.gpc('icon')} onClick={() => this.iconOnClick('right')}>
          <Icon type="right" />
        </div>
  ...
  private viewQuickJumper = () => {
    ...
    return (
      <div className={this.gpc('quick-jumper')}>
        跳至
